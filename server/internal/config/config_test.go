...
func TestLoadRejectsProductionWithoutOTPPepper(t *testing.T) {
	setRequiredEnv(t)
	t.Setenv("APP_ENV", "prod")
	t.Setenv("MESSAGE_KEY", strings.Repeat("a", 64))
	t.Setenv("OTP_PEPPER", "short")

	if _, err := Load(); err == nil || !strings.Contains(err.Error(), "OTP_PEPPER") {
		t.Fatalf("Load() error = %v, want OTP_PEPPER validation", err)
	}
}
